...
def ssl_classifier_cost(p_logits_l, t_labels_l, p_logits_u, p_u,
    ...
    en_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=p_u, logits=p_logits_u))
...
def model(observed, n, n_x, n_y, n_z, is_training):
    with zs.StochasticGraph(observed=observed) as model:
        ...
        lx_zy = layers.fully_connected(tf.concat([z, y], 1), 500,
                                       activation_fn=tf.nn.softplus,
                                       normalizer_fn=layers.batch_norm,
                                       normalizer_params=normalizer_params)
        lx_zy = layers.fully_connected(tf.concat([lx_zy, y], 1), 500,
                                       activation_fn=tf.nn.softplus,
                                       normalizer_fn=layers.batch_norm,
                                       normalizer_params=normalizer_params)
        x = layers.fully_connected(lx_zy, n_x, activation_fn=tf.nn.sigmoid)
    return x, y, z
...
def qz_xy(x, y, n_z, is_training):
    ...
    lz_xy = layers.fully_connected(tf.concat([x, y], 1), 500,
                                   ...
                                   normalizer_fn=layers.batch_norm,
                                   normalizer_params=normalizer_params)
    lz_xy = layers.fully_connected(tf.concat([lz_xy, y], 1), 500,
                                   activation_fn=tf.nn.softplus,
                                   normalizer_fn=layers.batch_norm,
                                   normalizer_params=normalizer_params)
...
def qy_x(x, n_y, is_training):
    ...
    ly_x = layers.conv2d(layers.dropout(ly_x, 0.8, is_training=is_training),
                         32, 5, stride=2, padding='VALID',
                         normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params,  # (12, 12, 32)
                         weights_initializer=init_ops.RandomNormal(stddev=0.05))
    ly_x = layers.conv2d(ly_x, 64, 3, normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params,  # (12, 12, 64)
                         weights_initializer=init_ops.RandomNormal(stddev=0.05))
    ly_x = layers.conv2d(layers.dropout(ly_x, 0.5, is_training=is_training),
                         64, 3, stride=2, padding='VALID',  # (5, 5, 64)
                         normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params,
                         weights_initializer=init_ops.RandomNormal(stddev=0.05))
...
def discriminator(x, y, z, n_y, n_z, is_training):
    ...
    lc_x = max_out(layers.conv2d(
        ...
        init_ops.RandomNormal(stddev=0.01)), num_pieces)  # 28
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        64, 4, stride=2, padding='VALID', activation_fn=None,
        weights_initializer=init_ops.RandomNormal(stddev=0.01)), num_pieces)  # 13
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        128, 4, padding='VALID', activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)  # 10
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        256, 4, stride=2, padding='VALID', activation_fn=None,
        weights_initializer=init_ops.RandomNormal(stddev=0.01)), num_pieces)  # 4
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        512, 3, padding='VALID', activation_fn=None, weights_initializer=
...
if __name__ == "__main__":
    ...
    def build_tower_graph(x_l, y_l, x_u, id_):
        ...
        x_u, y_u, z_u, qy_u, qy_logits_u = unlabeled(tower_x_u, n_x, n_y, n_z,
                                                     is_training)
        ...
        cls_loss = beta * ssl_classifier_cost(
            qy_logits_l, tower_y_l, qy_logits_u, qy_u)
    ...
    eval_x_gen, _, _ = model({'y': y_labeled_ph},
                             batch_size, n_x, n_y, n_z, is_training)
        ...
        for epoch in range(1, epoches + 1):
            ...
            for t in range(iters):
                ...
                if iter % test_freq == 0:
                    ...
                    recon_images = np.reshape(recon_images, [-1, 28, 28, 1])
