...
def multi_gan(classifier, g_model, l_infer, u_infer):
    ...
    disc_loss = tf.reduce_sum(disc_losses_list)
    ...
    gen_loss = tf.reduce_sum(gen_losses_list)
...
def model(observed, n, n_x, n_y, n_z, is_training):
    with zs.StochasticGraph(observed=observed) as model:
        ...
        lx_z = layers.conv2d_transpose(lx_z, 128, 3, 1, activation_fn=lrelu,
        ...
        lx_z = layers.conv2d_transpose(lx_z, 32, 5, stride=2,
                                       padding='SAME', weights_initializer=
                                       init_ops.RandomNormal(stddev=0.01),
                                       activation_fn=lrelu,
                                       normalizer_fn=layers.batch_norm,
                                       normalizer_params=normalizer_params)
        ...
        lx_z = layers.conv2d(lx_z, 32, 1, stride=1, activation_fn=lrelu,
                             ...
                             normalizer_params=normalizer_params)
        x = layers.conv2d(lx_z, 1, 1, stride=1, activation_fn=tf.nn.sigmoid,
                          weights_initializer=
                          init_ops.RandomNormal(stddev=0.01))
    return x, y, z
...
def qz_xy(x, y, n_z, is_training):
    ...
    lz_x = layers.conv2d(tf.concat([x, y], -1), 32, 5, stride=2, activation_fn=lrelu,
...
def qy_x(x, n_y, is_training):
    ...
    ly_x = layers.conv2d(ly_x, 256, 3, padding='VALID',
                         normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params)
    ly_x = layers.conv2d(ly_x, 128, 1, padding='VALID',
                         normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params)
    ly_x = layers.conv2d(ly_x, 64, 1, padding='VALID',
                         normalizer_fn=layers.batch_norm,
                         normalizer_params=normalizer_params)
...
def discriminator(x, y, z, n_y, n_z, is_training):
    ...
    lc_x = max_out(layers.conv2d(
        ...
        32, 5, stride=2, padding='SAME', activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        64, 5, stride=2, padding='SAME', activation_fn=None,
    ...
    lc_xyz = max_out(layers.conv2d(
        ...
        768, 1, activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_xyz = max_out(layers.conv2d(
        layers.dropout(lc_xyz, 0.5, is_training=is_training),
        768, 1, activation_fn=None, weights_initializer=
...
if __name__ == "__main__":
    ...
    result_path = '/home/mengxi/logs/multi_gan_mnist'
