...
def q_net(n_x, n_y, n_z, n_samples):
    ...
    with pt.defaults_scope(activation_fn=tf.nn.relu):
        ...
        ly_28_28 = PrettyTensor({'y_dim4': sym_y_dim4},
                                pt.template('y_dim4').apply(tf.tile,
                                                            [1, 28, 28, 1]))
        ly_14_14 = PrettyTensor({'y_dim4': sym_y_dim4},
                                pt.template('y_dim4').apply(tf.tile,
                                                            [1, 14, 14, 1]))
        ly_7_7 = PrettyTensor({'y_dim4': sym_y_dim4},
                              pt.template('y_dim4').apply(tf.tile,
                                                          [1, 7, 7, 1]))
        lz_xy = PrettyTensor({'x': lx, 'y_28': ly_28_28, 'y_14': ly_14_14,
                              'y_7': ly_7_7},
                             pt.template('x').
                             reshape([-1, 28, 28, 1]).
                             concat(3, [pt.template('y_28')]).
                             conv2d(5, 32, stride=2).
                             # batch_normalize(scale_after_normalization=True).
                             concat(3, [pt.template('y_14')]).
                             conv2d(5, 64, stride=2).
                             # batch_normalize(scale_after_normalization=True).
                             concat(3, [pt.template('y_7')]).
