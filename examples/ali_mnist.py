...
flags = tf.app.flags
flags.DEFINE_boolean("is_run_on_server", True, "True for running on server, Flase for running on local")
local_FLAG = flags.FLAGS
...
def decoder(observed, n, n_z=64, is_training=True):
    with zs.StochasticGraph(observed=observed) as decoder:
        ...
        lx_z = layers.conv2d_transpose(lx_z, 128, 3, 1,
                                       activation_fn=lrelu,
                                       padding='VALID',
        ...
        lx_z = layers.conv2d_transpose(lx_z, 64, 5, stride=1,
                                       padding='VALID',
                                       activation_fn=lrelu,
                                       normalizer_fn=layers.batch_norm,
                                       normalizer_params=normalizer_params,
        ...
        lx_z = layers.conv2d_transpose(lx_z, 32, 5, stride=2,
                                       activation_fn=lrelu,
                                       normalizer_fn=layers.batch_norm,
                                       normalizer_params=normalizer_params,
                                       weights_initializer=
        ...
        lx_z = layers.conv2d(lx_z, 1, 1, stride=1, activation_fn=tf.nn.sigmoid,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
...
def encoder(observed, x, n_z, is_training):
    with zs.StochasticGraph(observed=observed) as encoder:
        normalizer_params = {'is_training': is_training,
                             'updates_collections': None}
        lz_x = layers.conv2d(x, 32, 5, stride=2, activation_fn=lrelu,
                            normalizer_fn=layers.batch_norm,
                             normalizer_params=normalizer_params,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
        lz_x = layers.conv2d(lz_x, 64, 5, stride=2, activation_fn=lrelu,
                             normalizer_fn=layers.batch_norm,
                             normalizer_params=normalizer_params,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
        lz_x = layers.conv2d(lz_x, 128, 5, stride=1, activation_fn=lrelu,
                             padding='VALID', normalizer_fn=layers.batch_norm,
                             normalizer_params=normalizer_params,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
        ...
        lz_x = layers.conv2d(lz_x, 256, 3, stride=1, activation_fn=lrelu,
                            padding='VALID', normalizer_fn=layers.batch_norm,
                            normalizer_params=normalizer_params,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
        lz_x = layers.conv2d(lz_x, 256, 1, stride=1, activation_fn=lrelu,
                             padding='VALID', normalizer_fn=layers.batch_norm,
                             normalizer_params=normalizer_params,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
        lz_x = layers.conv2d(lz_x, 80, 1, stride=1, activation_fn=None,
                             weights_initializer=
                             init_ops.RandomNormal(stddev=0.01)
                             )
...
def discriminator(x, z, is_training):
    ...
    lc_x = max_out(layers.conv2d(
        ...
        32, 5, stride=2, activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        64, 5, stride=2, activation_fn=None,
        weights_initializer=init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        128, 5, padding='VALID', activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_x = max_out(layers.conv2d(
        layers.dropout(lc_x, 0.5, is_training=is_training),
        256, 3,padding='VALID', activation_fn=None,
        weights_initializer=init_ops.RandomNormal(stddev=0.01)), num_pieces)
    ...
    lc_z = max_out(layers.conv2d(
        ...
        256, 1, activation_fn=None, weights_initializer=
        init_ops.RandomNormal(stddev=0.01)), num_pieces)
    lc_z = max_out(layers.conv2d(
...
if __name__ == "__main__":
    ...
    # Load MNIST
    data_path = os.path.join('/home/mengxi/datasets/MNIST_DATA',
                             'mnist.pkl.gz') if local_FLAG.is_run_on_server else \
        os.path.join('/Users/Popcorn/Documents/coding/ZhuSuan/examples/data',
                             'mnist.pkl.gz')
    ...
    x_test = np.reshape(x_train, [-1,n_xl, n_xl, 1])
    ...
    result_path = '/home/mengxi/logs/ali_mnist' if local_FLAG.is_run_on_server else \
        '/Users/Popcorn/Documents/coding/ali_mnist'
        ...
        for epoch in range(1, epoches + 1):
            ...
            for t in range(iters):
                ...
                if iter % test_freq == 0:
                    ...
                    name = "gen/ali_mnist.epoch.{}.iter.{}.png".format(epoch, iter)
                    ...
                    name = "recon/ali_mnist.epoch.{}.iter.{}.png".format(
                        epoch, iter)
                    name = os.path.join(
                        result_path, name)
                ...
                if iter % save_freq == 0:
                    save_path = "ali_mnist.epoch.{}.iter.{}.ckpt".format(epoch, iter)
