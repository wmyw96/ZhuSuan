...
class M2(object):
    ...
    def p_net(self):
        with pt.defaults_scope(activation_fn=tf.nn.relu):
            ...
            y_4_4 = sym_y_dim4.apply(tf.tile, [1, 4, 4, 1])
            y_8_8 = sym_y_dim4.apply(tf.tile, [1, 8, 8, 1])
            y_16_16 = sym_y_dim4.apply(tf.tile, [1, 16, 16, 1])
            l_x_zy = (pt.template('z').join([sym_y]).
                      reshape([-1, self.n_z + self.n_y]).
                      fully_connected(128*4*4).
                      reshape([-1, 4, 4, 128]).
                      concat(concat_dim=3, other_tensors=[y_4_4]).
                      deconv2d(5, 32, stride=2, batch_normalize=False).
                      concat(concat_dim=3, other_tensors=[y_8_8]).
                      deconv2d(5, 16, stride=2, batch_normalize=False).
                      concat(concat_dim=3, other_tensors=[y_16_16]).
    ...
    def log_prob(self, latent, observed, given):
        ...
        log_px_zy = tf.log(logistic.cdf(x + 1. / 256, l_x_zy, x_scale) -
                           logistic.cdf(x, l_x_zy, x_scale) + 1e-8)
        log_px_zy = tf.reduce_sum(log_px_zy, 2)
...
def q_net(n_x, n_y, n_z, n_samples):
    ...
    with pt.defaults_scope(activation_fn=tf.nn.relu):
        ...
        sym_y_dim4 = PrettyTensor({'y': ly},
                             pt.template('y').reshape([-1, 1, 1, n_y]))
        ly_32_32 = PrettyTensor({'y_dim4': sym_y_dim4},
                                pt.template('y_dim4').apply(tf.tile,
                                                            [1, 32, 32, 1]))
        ly_16_16 = PrettyTensor({'y_dim4': sym_y_dim4},
                                pt.template('y_dim4').apply(tf.tile,
                                                            [1, 16, 16, 1]))
        ly_8_8 = PrettyTensor({'y_dim4': sym_y_dim4},
                              pt.template('y_dim4').apply(tf.tile,
                                                          [1, 8, 8, 1]))
        lz_xy = PrettyTensor({'x': lx, 'y_32': ly_32_32, 'y_16': ly_16_16,
                              'y_8': ly_8_8},
                             pt.template('x').
                             reshape([-1, 32, 32, 3]).
                             concat(3, [pt.template('y_32')]).
                             conv2d(5, 32, stride=2).
                             # batch_normalize(scale_after_normalization=True).
                             concat(3, [pt.template('y_16')]).
                             conv2d(5, 64, stride=2).
                             # batch_normalize(scale_after_normalization=True).
                             concat(3, [pt.template('y_8')]).
...
if __name__ == "__main__":
    ...
    cost = -(labeled_lower_bound + unlabeled_lower_bound -
             classifier_cost) / 2.
    bits_per_dim_u = -unlabeled_lower_bound / n_x * 1. / np.log(2.)
    bits_per_dim_l = -labeled_lower_bound / n_x * 1. / np.log(2.)
        ...
        for epoch in range(1, epoches + 1):
            ...
            for t in range(iters):
                ...
                _, lb_labeled, lb_unlabeled, train_acc = sess.run(
                    [infer, bits_per_dim_l, bits_per_dim_u, acc],
            ...
            if epoch % test_freq == 0:
                ...
                for t in range(test_iters):
                    ...
                    test_ll_labeled, test_ll_unlabeled, test_acc = sess.run(
                        [bits_per_dim_l, bits_per_dim_u, acc],
